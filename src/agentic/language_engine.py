...
VLLM_CONFIG = {
    "model_path": "Qwen/Qwen3-8B",         # "HuggingFaceTB/SmolLM2-1.7B-Instruct" or "Qwen3-4B" or "Qwen3-4B-Instruct-2507" or "Qwen/Qwen3-0.6B",
...
class LanguageEngine:
    ...
    def __init__(self, config=VLLM_CONFIG):
        ...
        # Infer max_model_len from tokenizer if not explicitly set, which is needed for YaRN rope scaling
        if self.config["max_model_len"] is None:
            if hasattr(self.tokenizer, "model_max_length"):
                self.config["max_model_len"] = self.tokenizer.model_max_length
            else:
                raise ValueError("max_model_len must be specified in config if tokenizer does not have model_max_length attribute.")

        if "rope_scaling" in model_org_config:
            org_rope_scaling = model_org_config.rope_scaling
            print(f"Model config already has rope_scaling settings: {org_rope_scaling}")
        else:
            print("Model config does not have rope_scaling settings.")
            org_rope_scaling = None
        
        if self.config["yarn_rope_scaling"] and org_rope_scaling is None: # We have to set up and ensure that the rope_scaling config is not already present in the model config, otherwise vLLM will ignore our custom rope scaling settings
            print("Initializing model with YaRN rope scaling enabled...")
            _yarn_config, new_max_len = self.__construct_yarn_config()
            self.model = LLM(
                model=self.config["model_path"],
                tensor_parallel_size=self.config["tensor_parallel_size"],
                gpu_memory_utilization=self.config["gpu_memory_utilization"],
                max_model_len=new_max_len,
                dtype=self.config["dtype"],
                trust_remote_code=True,
                enforce_eager=True,
                hf_overrides=_yarn_config
            )
        else:
            print("Initializing model without YaRN rope scaling...")
            self.model = LLM(
                model=self.config["model_path"],
                tensor_parallel_size=self.config["tensor_parallel_size"],
                gpu_memory_utilization=self.config["gpu_memory_utilization"],
    ...
    def __construct_yarn_config(self):
        ...
        # print(f"Original model config loaded for {self.config['model_path']}: {config}")
        # exit()
        ...
        print(f"Constructed YaRN config for {self.config['model_path']}: {yarn_config}")
        new_max_len = int(original_max_position_embeddings * factor)
        print(f"With YaRN scaling, the new max model length is: {new_max_len}")
        return yarn_config, new_max_len
