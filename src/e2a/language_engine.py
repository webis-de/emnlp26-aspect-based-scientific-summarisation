...
class LanguageEngine:
    ...
    def generate_structured_in_batch(self, user_prompts, pydantic_model, enable_thinking=False, system_prompt=None):
        ...
        raw_results, _ = self.__generate_structured_in_batch(user_prompts, json_schema, enable_thinking, system_prompt)
        outputs = []
    ...
    def generate_structured_in_batch_with_token_metadata(self, user_prompts, pydantic_model, enable_thinking=False, system_prompt=None):
        ...
        """
        json_schema = pydantic_model.model_json_schema()
        raw_results, raw_outputs = self.__generate_structured_in_batch(user_prompts, json_schema, enable_thinking, system_prompt)
        parsed = []
        token_metadata = []
        for idx, (result, raw_output) in enumerate(zip(raw_results, raw_outputs)):
            ...
            try:
                ...
                if repaired != result:
                    try:
                        parsed.append(pydantic_model.model_validate_json(repaired))
                        continue
                    except Exception:
                        pass
